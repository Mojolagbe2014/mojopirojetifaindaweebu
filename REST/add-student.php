...
if(filter_input(INPUT_POST, "register") != NULL){
    $postVars = array('name','matricNo','email','department','phone', 'passWord', 'passWord2'); // Form fields names
    //Validate the POST variables and add up to error message if empty
    foreach ($postVars as $postVar){
        switch($postVar){
            case 'passWord2':   if(filter_input(INPUT_POST,$postVar) !== filter_input(INPUT_POST, "passWord")){
                                array_push ($errorArr, "Password Mismatch !!! ");
                                if(filter_input(INPUT_POST, $postVar) == "") {array_push ($errorArr, " confirm your password. ");}}
                                break;
            case 'passWord'     :   $studentObj->passWord = filter_input(INPUT_POST, $postVar) ? mysqli_real_escape_string($dbObj->connection, filter_input(INPUT_POST, $postVar)) :  ''; 
                            if($studentObj->passWord == "") {array_push ($errorArr, " $postVar ");}
                            break;
...
if(filter_input(INPUT_GET, "register") != NULL){
    ...
    foreach ($postVars as $postVar){
        switch($postVar){
            case 'passWord2':   if(filter_input(INPUT_GET,$postVar) !== filter_input(INPUT_GET, "passWord")){
                                array_push ($errorArr, "Password Mismatch !!! ");
                                if(filter_input(INPUT_GET, $postVar) == "") {array_push ($errorArr, " confirm your password. ");}}
                                break;
            case 'passWord'     :   $studentObj->passWord = filter_input(INPUT_GET, $postVar) ? mysqli_real_escape_string($dbObj->connection, filter_input(INPUT_GET, $postVar)) :  ''; 
                            if($studentObj->passWord == "") {array_push ($errorArr, " $postVar ");}
                            break;
