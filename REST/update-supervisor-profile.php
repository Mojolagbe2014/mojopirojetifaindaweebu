...
if(filter_input(INPUT_POST, "update") != NULL){
    ...
    foreach ($postVars as $postVar){
        switch($postVar){
            case 'id':   $supervisorObj->$postVar = filter_input(INPUT_POST, $postVar, FILTER_VALIDATE_EMAIL) ? mysqli_real_escape_string($dbObj->connection, filter_input(INPUT_POST, $postVar, FILTER_VALIDATE_EMAIL)) :  ''; 
                            if($supervisorObj->$postVar === "") {array_push ($errorArr, "Please enter valid email ");}
                            break;
...
if(filter_input(INPUT_GET, "update") != NULL){
    ...
    foreach ($postVars as $postVar){
        switch($postVar){
            case 'id':   $supervisorObj->$postVar = filter_input(INPUT_GET, $postVar, FILTER_VALIDATE_EMAIL) ? mysqli_real_escape_string($dbObj->connection, filter_input(INPUT_GET, $postVar, FILTER_VALIDATE_EMAIL)) :  ''; 
                            if($supervisorObj->$postVar === "") {array_push ($errorArr, "Please enter valid email ");}
                            break;
